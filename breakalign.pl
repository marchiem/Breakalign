...
my $k_d = 0; # option to keep just the database of all reads in input; to avoid building it for each site tested
...
if ($help)
{
	...
	print "ESSENTIAL\n";
	print "-r   FILE    File containing reads\n";
		...
		elsif($bed_co) # in this case we use a bed file with set of coordinates
		{
			...
			while(<BCO>)
			{
			  ...
			  unless($line_co) {next;}
				...
				# I am repeating this later; maybe it should be done once at the beginning of the code
			...
			exit; # once we have run breakalign for each line coordinate we can exit
